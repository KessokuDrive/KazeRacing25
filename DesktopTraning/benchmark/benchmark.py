...
Usage:
    python3 benchmark/benchmarkAlt.py --model model_trt/kazeRacing002_trt.pth --images_folder benchmark/dataset --model_type resnet18 --output_folder ./output --use_trt
"""
...
def save_output_image(image, output, frame_number, output_folder, raw_output=None):
    ...
    try:
        if len(output) >= 2:
            ...
            x_scaled = (x_raw + 1)/2 * (width - 1)
            y_scaled = (y_raw + 1)/2 * (height - 1)
